...
internal static unsafe class RapierBindings
{
	...
	public static void RemoveCollider(ColliderHandle handle) => ((delegate* unmanaged[Cdecl]<ColliderHandle, void>) data.Data.removeCollider)(handle);
#else
	...
	[DllImport(DllName, CallingConvention = Convention, EntryPoint="remove_collider")]
	public static extern unsafe void RemoveCollider(ColliderHandle handle);
#endif
#if UNITY_EDITOR && !DISABLE_DYNAMIC_RAPIER_LOAD
	...
	public static bool CastCuboid(float fromX, float fromY, float fromZ, float dirX, float dirY, float dirZ, float halfExtentsX, float halfExtentsY, float halfExtentsZ, RapierShapecastOptions options, RapierShapecastHit* outHit) => ((delegate* unmanaged[Cdecl]<float, float, float, float, float, float, float, float, float, RapierShapecastOptions, RapierShapecastHit*, bool>) data.Data.castCuboid)(fromX, fromY, fromZ, dirX, dirY, dirZ, halfExtentsX, halfExtentsY, halfExtentsZ, options, outHit);
#else
	...
	[DllImport(DllName, CallingConvention = Convention, EntryPoint="cast_cuboid")]
	public static extern unsafe bool CastCuboid(float fromX, float fromY, float fromZ, float dirX, float dirY, float dirZ, float halfExtentsX, float halfExtentsY, float halfExtentsZ, RapierShapecastOptions options, RapierShapecastHit* outHit);
#endif
#if UNITY_EDITOR && !DISABLE_DYNAMIC_RAPIER_LOAD
    ...
    struct UnmanagedData
    {
        ...
        public void load_calls()
        {
			...
			removeCollider = NativeLoader.GetFunction(loaded_lib, "remove_collider");
			...
			castCuboid = NativeLoader.GetFunction(loaded_lib, "cast_cuboid");
